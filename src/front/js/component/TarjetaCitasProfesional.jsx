...
export const TarjetaCitasProfesional = ({ appointment }) => {
  ...
  const appointmentLocalDate = new Date(appointment.availability?.date).toLocaleDateString("es-UY", { timeZone: "UTC" })
  const currentLocalDate = new Date().toLocaleDateString("es-UY", { timeZone: "UTC" })

  const isAppointmentPast = appointmentLocalDate < currentLocalDate
